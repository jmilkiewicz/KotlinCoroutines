...
class FetchTasksUseCase(
    ...
    suspend fun fetchTasks(): List<Task> {
        return suspendCancellableCoroutine { continuation ->
            val cancellable = callbackUseCase.fetchTasks({ tasks -> continuation.resume(tasks) }) { error ->
    ...
    suspend fun fetchTasksResult(): Result<List<Task>> {
        return suspendCancellableCoroutine { continuation ->
            val cancellable =
                callbackUseCase.fetchTasks({ tasks -> continuation.resume(Result.success(tasks)) }) { error ->
                    continuation.resume(
    ...
    suspend fun fetchTasksOrNull(): List<Task>? {
        return suspendCancellableCoroutine { continuation ->
            val cancellable = callbackUseCase.fetchTasks({ tasks ->
                if (tasks.isEmpty()) {

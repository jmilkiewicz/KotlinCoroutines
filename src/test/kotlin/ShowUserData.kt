...
class ShowUserDataUseCase(
    ...
    suspend fun show() {
        val profileDeferred = notificationScope.async {
            repo.getProfile()
        }
        val friendsDeferred = notificationScope.async {
            repo.getFriends()
        }
        val nameDeferred = notificationScope.async {
            repo.getName()
        }

        val user = User(nameDeferred.await(), friendsDeferred.await(), profileDeferred.await())


        view.show(user)
        notificationScope.launch { repo.notifyProfileShown() }

        //czy to nie będzie czekać na to aby user został w pełnie pobrany
